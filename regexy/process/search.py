...
def _search(nfa: NFA, text: Iterator[str]) -> Union[MatchedType, None]:
    """
    :param nfa: a NFA
...
def slice(text, start) -> Iterator[str]:
    index = start
...
def search(nfa: NFA, text: str) -> Union[MatchedType, None]:
    for start in range(len(text)):
        match = _search(nfa, slice(text, start))

        if match is not None:
            return match


def parallel_search(nfa: NFA, text: Iterator[str]) -> Union[MatchedType, None]:
    """
    This is O(n*m^2) time
